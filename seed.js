...
const createSubjects = async (common, js, ss) => {
	...
	const commonSubjectsPromises = commonSubjects.map((name) =>
		createSubject({ name, type: 'COMMON' })
	);
	const jssSubjectsPromises = jssSubjects.map((name) =>
		createSubject({ name, type: 'JSS' })
	);
	const sssSubjectsPromises = sssSubjects.map((name) =>
		createSubject({ name, type: 'SSS' })
	);

	await Promise.all([
		...commonSubjectsPromises,
		...jssSubjectsPromises,
		...sssSubjectsPromises,
	]);
...
const clearDatabase = async () => {
	await Promise.all([deleteAllStudents(), deleteAllSubjects()]);
};
...
const run = async () => {
	...
	for (let i = 0; i < TOTAL_NUMBER_OF_STUDENTS; i += 1) {
		...
		const subjectPool = JSS_CLASSES.includes(className)
			? [...common, ...jss]
			: [...common, ...sss];
